...
(function($){
	...
	$.supersized = function( options ) {
		...
		if (options.slides.length > 1){
			...
			var imageId   = (options.slides[loadPrev].id)  ? options.slides[loadPrev].id                   : "";
			$("<img/>").attr("src", options.slides[loadPrev].image).attr("id" , imageId).appendTo(element).wrap('<a ' + imageLink + linkTarget + '></a>');
		}

		//Set current image
		imageLink = (options.slides[currentSlide].url) ? "href='" + options.slides[currentSlide].url + "'" : "";
		imageId   = (options.slides[currentSlide].id)  ? options.slides[currentSlide].id                   : "";
		$("<img/>").attr("src", options.slides[currentSlide].image).attr("id" , imageId).appendTo(element).wrap('<a class="activeslide" ' + imageLink + linkTarget + '></a>');
		...
		if (options.slides.length > 1){
			...
			imageId   = (options.slides[loadNext].id)  ? options.slides[loadNext].id                   : "";
			$("<img/>").attr("src", options.slides[loadNext].image).attr("id" , imageId ).appendTo(element).wrap('<a ' + imageLink + linkTarget + '></a>');
		}
		...
		function nextslide() {
			...
			imageId   = (options.slides[loadSlide].id ) ? options.slides[loadSlide].id                   : "";
			$("<img/>").attr("src", options.slides[loadSlide].image).attr("id" , imageId ).appendTo(element).wrap("<a " + imageLink + linkTarget + "></a>");	//Append new image
			...
			if( typeof options.beforeSlide == "function" ){
				options.beforeSlide( options.slides[currentSlide] );
			}
		...
		function afterAnimation() {
			...
			if( typeof options.afterAnimation == "function" ){
				options.afterAnimation( options.slides[currentSlide] );
			}
		}
